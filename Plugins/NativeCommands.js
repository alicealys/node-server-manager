...
var plugin = {
    ...
    playerCommand: function (Player, args) {
        ...
        var commands = {
          ...
          'stats': {
            ...
            callback: async function (Player, args) {
              var ClientId = !args[1] ? Player.ClientId : args[1]
              var Stats = await plugin.Server.DB.getPlayerStatsTotal(ClientId)
              var OtherStats = await plugin.Server.DB.getPlayerStats(ClientId)
              if (Stats)
          ...
          'token': {
            ...
            callback: async (Player) => {
              ...
              await plugin.Server.DB.createToken(Player.ClientId, rawToken.join(''))
            }
          },
          'rcon': {
            ArgumentLength: 1,
            ...
            callback: async function (Player, args) {
              var result = (await plugin.Server.Rcon.executeCommandAsync(args.slice(1).join(' '))).split('\n')
          ...
          'tp': {
            ...
            Permission: Permissions.Commands.COMMAND_TP,
            callback: async function (Player, args) {
              var Client = await plugin.Server.DB.getClient(args[1])
              var Target = await plugin.Server.Rcon.getClientByName(Client.Name)
              switch (true) {
                case !Client:
                case !Target:
              ...
              await plugin.Server.Rcon.executeCommandAsync(`seta tp_src ${Player.Clientslot}`)
              await plugin.Server.Rcon.executeCommandAsync(`seta tp_dest ${Target.Clientslot}`)
          ...
          'tphere': {
            ...
            callback: async function (Player, args) {
              ...
              await plugin.Server.Rcon.executeCommandAsync(`seta tp_src ${Target.Clientslot}`)
              await plugin.Server.Rcon.executeCommandAsync(`seta tp_dest ${Player.Clientslot}`)
          ...
          'owner': {
            Permission: Permissions.Commands.COMMAND_USER_CMDS,
            callback: async (Player) => {
              var Owner = await plugin.Server.DB.getOwner()
              console.log(Owner)
              switch (true) {
                case !Owner:
                  plugin.Server.DB.setLevel(Player, 5)
                ...
                case (Owner.ClientId == Player.ClientId):
                  ...
                  return;
                case (Owner.ClientId != Player.ClientId):
                  Player.Tell(`${plugin.Server.DB.getClient(Owner.ClientId).Name} owns this server`)
                  return;
              }
            }
          },
          ...
          'find': {
            ...
            Permission: Permissions.Commands.COMMAND_USER_CMDS,
            callback: async (Player, args) => {
               var MatchedClients = await plugin.Server.DB.getClientByName(args.slice(1).join(' '))
    ...
    getRoleFrom: function(Value, Type) {
      switch (Type) {
        case 0:
    ...
    onEventAsync: async function(event) {
      switch (event.type) {
          case 'say':
            if (event.data.Message.startsWith('.')) plugin.playerCommand(event.data.Origin, event.data.Message.substr(1).split(/\s+/))
          break;
      }
    },
    init: function() {
        this.Server.on('event', this.onEventAsync);
    },
